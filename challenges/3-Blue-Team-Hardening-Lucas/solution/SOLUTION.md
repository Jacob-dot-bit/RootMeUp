...
### Q1 — SSH root login → `RootMeUp{r00t_l0gin_disabled}`
```bash
sudo sed -i 's/^PermitRootLogin .*/PermitRootLogin no/' /etc/ssh/sshd_config
```
### Q2 — Mots de passe vides → `RootMeUp{n0_empty_passw0rds}`
```bash
sudo sed -i 's/^PermitEmptyPasswords .*/PermitEmptyPasswords no/' /etc/ssh/sshd_config
```
### Q3 — /etc/shadow → `RootMeUp{shadow_l0cked_down}`
```bash
...
### Q4 — Secret applicatif → `RootMeUp{secrets_perms_600}`
Le fichier `/opt/app/.env` est lisible par tous.
```bash
sudo chmod 600 /opt/app/.env
```
### Q5 — Persistance planifiée → `RootMeUp{malicious_cron_purged}`
Une tâche cron rappelle un C2.
```bash
sudo rm /etc/cron.d/sysupdate
```
### Q6 — Service réseau → `RootMeUp{telnet_is_dead}`
...
### Q7 — Accès admin caché → `RootMeUp{no_hidden_r00t_user}`
...
```
### Q8 — Élévation de privilèges → `RootMeUp{suid_backdoor_cleared}`
...
### Q9 — Intégrité du PATH → `RootMeUp{writable_path_secured}`
...
```
### Q10 — Accès distant résiduel → `RootMeUp{ssh_backdoor_key_removed}`
