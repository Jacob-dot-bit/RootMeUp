...
> Instance*, image `rootmeup/bt3-hardening:1.0`, port `8000`). **Flags 2 → 10** =
> type *standard*, chacun avec en **Prérequis** la question précédente (déblocage
> progressif). Le joueur obtient chaque flag avec `getflag <N>` dans le terminal
> une fois la faille corrigée, puis le saisit dans CTFd.

---

## Flag 1 — Instance et Flag 1 : Verrouiller l'accès SSH root
**Points : 50** · Type : *container* (image `rootmeup/bt3-hardening:1.0`, port 8000)
...
de repérer des backdoors subtiles, **sans indice**. Format : `RootMeUp{...}`.
...
**Flag :** `RootMeUp{r00t_l0gin_disabled}`
...
**Flag :** `RootMeUp{n0_empty_passw0rds}`
...
**Flag :** `RootMeUp{shadow_l0cked_down}`
...
**Flag :** `RootMeUp{secrets_perms_600}`
...
**Flag :** `RootMeUp{malicious_cron_purged}`
...
**Flag :** `RootMeUp{no_hidden_r00t_user}`
...
**Flag :** `RootMeUp{suid_backdoor_cleared}`
...
**Flag :** `RootMeUp{writable_path_secured}`
...
**Flag :** `RootMeUp{ssh_backdoor_key_removed}`
